...
const PlayerScores = ({ scores }: Scores) => {
    ...
    const lines = Array.from({length: lineCount}, (x, i) => (
        <line y1="0" y2={scores.length * 40} x1={i * tickWidth} x2={i * tickWidth} style={{"stroke": "#D2D6DF"}} />
    ));
    ...
    const scoreBars = scores.map((x, i) => (
        <rect height="36" x="0" y={i * 40} width={scale(x.Score, 0, maxScore, 0, graphWidth)} fill={x.Team === 0 ? '#00179e' : '#c65209'} />
    ));

    const scoreLabels = pureScores.map((x, i) => (
        <g transform={`translate(${3 + scale(x, 0, maxScore, 0, graphWidth)}, ${24 + i * 40})`}>
            <text>{x}</text>
        </g>
    ));
